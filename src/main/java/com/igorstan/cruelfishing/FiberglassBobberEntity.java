...
import com.igorstan.cruelfishing.Entities;
import top.theillusivec4.combustivefishing.common.registry.CombustiveFishingLoot;

public class FiberglassBobberEntity extends FishingBobberEntity {
    ...
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager
            .createKey(FiberglassBobberEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> DATA_CATCHING = EntityDataManager
            .createKey(FiberglassBobberEntity.class, DataSerializers.BOOLEAN);

    private final Random random = new Random();
    private final int luck;
    private final int lureSpeed;
    private FiberglassBobberEntity.State currentState = FiberglassBobberEntity.State.FLYING;
    ...
    @OnlyIn(Dist.CLIENT)
    public FiberglassBobberEntity(World worldIn) {
        super(worldIn, Minecraft.getInstance().player, 0, 0, 0);
        this.luck = 0;
        this.lureSpeed = 0;
    }

    public FiberglassBobberEntity(PlayerEntity fishingPlayer, World worldIn, int luck,
                                      int lureSpeed) {
    ...
    public EntityType<?> getType() {
        return Entities.FIBERGLASS_BOBBER;
    }
    ...
    public void tick() {
        ...
        if (playerentity == null) {
            ...
            if (fluidstate.getFluid() == Fluids.OIL_FLUID.get()) {
    ...
    private LavaType getLavaType(BlockPos pos) {
        ...
        if (!blockstate.isAir(this.world, pos) && !blockstate.isIn(Blocks.LILY_PAD)) {
            ...
            return fluidstate.getFluid() == Fluids.OIL_FLUID.get() && fluidstate.isSource() && blockstate
    ...
    private boolean shouldStopFishing(PlayerEntity playerEntity) {
        ...
        boolean flag = itemstack.getItem() instanceof FiberglassFishingRod;
        boolean flag1 = itemstack1.getItem() instanceof FiberglassFishingRod;
